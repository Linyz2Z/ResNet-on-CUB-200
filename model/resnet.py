...
class ResNet(nn.Module):
    def __init__(
        ...
        # 在 PyTorch 中为一个神经网络（特别是卷积神经网络）进行初始化。
        # 它在一个 PyTorch 的 nn.Module 网络的所有子模块上进行遍历，并根据模块的类型执行相应的参数初始化。
...
def resnet18(pretrained=False, progress=True, **kwargs):    # **kwargs 是一种语法，用来捕获函数调用中未明确列出的其他关键字参数。
    """Construct 18 layer Resnet model as in                # kwargs 是一个字典，其中包含所有未明确列出的关键字参数及其对应的值。
